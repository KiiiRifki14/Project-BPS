---
version: 2.0.0
name: Sistem Administrasi Keuangan dan Pengarsipan Digital (SAKDI)
description: >
  Design system resmi SAKDI untuk instansi Badan Pusat Statistik (BPS).
  Mengikuti identitas visual BPS (biru, hijau, oranye), standar aksesibilitas
  WCAG 2.1 AA, proporsi tipografi Perfect Fourth, spacing Fibonacci 8-point grid,
...
# ─────────────────────────────────────────────────────────────────
# 3. SPACING — Fibonacci 8-point Grid
# ─────────────────────────────────────────────────────────────────
spacing:
  0:   "0px"
  1:   "4px"    # ikon ↔ label
...
SAKDI adalah sistem administrasi keuangan dan pengarsipan digital untuk instansi BPS. Identitas visual bersumber langsung dari tiga warna logo resmi BPS — biru (kepercayaan/data), oranye (aksi/ekonomi), hijau (konfirmasi/pertanian) — yang juga melambangkan tiga kegiatan sensus besar BPS.
...
Fibonacci 8-point grid. Semua jarak adalah nilai dari token spacing (4–96px). **Dilarang** menggunakan nilai arbitrer seperti `15px` atau `22px`.
...
Format yang diizinkan: PDF, XLSX, DOCX, JPG, PNG. Batas: 10MB/file, 50MB/batch. Tampilkan progress upload dan state error per file.
